...
{-
Developer Notes:
The problem with this module is that it 
does not cover all cases. It cannot handle all
reasonable combinations of IPA graphemes.

It has a very strong dependency on the layout of the 
IPA chart, and so if the layout changes the source code 
would become more difficult to understand. 
Also, due to this dependency on the chart, there 
are many index out of bounds errors that really 
are unnecessary if we changed the way we did things. This
problem is
due to this use of the IPA chart as a list of lists that
we index into.

Finally, the code is tricky to reason about, and
often adding code to handle an IPA diacritic leads
to non-termination.
 -}
import Lib
...
constructIPA1 c@(Consonant VoicedAspirated place _ PulmonicEgressive) 
  | place `notElem` placesOnMainChart = constructIPA1 (deaspirate c) ++ "ʰ"
  | otherwise = constructUnaspiratedPulmonicEgressive (deaspirate c) ++ "ʰ"


constructIPA1 c@(Consonant Voiceless _ _ PulmonicEgressive) = 
  constructUnaspiratedPulmonicEgressive c

constructIPA1 c@(Consonant VoicelessAspirated place _ PulmonicEgressive)
  | place `notElem` placesOnMainChart = constructIPA1 (voicedPhonet (deaspirate c)) ++ "̥" ++ "ʰ"
  | otherwise = constructUnaspiratedPulmonicEgressive (voicedPhonet (deaspirate c)) ++ "̥" ++ "ʰ"
...
constructIPA2 (Consonant CreakyVoiced x y z) =
   constructIPA1 (Consonant Voiced x y z) ++ "̰" 

constructIPA2 (Vowel x y z CreakyVoiced) =
  constructIPA1 (Vowel x y z Voiced) ++ "̰" 
...
constructUnaspiratedPulmonicEgressive (Consonant voicing place manner PulmonicEgressive)
  | place `notElem` placesOnMainChart = ""
  | manner `notElem` mannersOnMainChart = ""
  | otherwise =
...
codeGenAllStatementsConstructIPAHaskell =
  let 
    construct_statements = map codeGenConstructIPAHaskell (generateFromUnmarked (Consonant UnmarkedVocalFolds UnmarkedPlace UnmarkedManner PulmonicEgressive))
    analyze_statements = map codeGenAnalyzeIPAHaskell (map constructIPA (generateFromUnmarked (Consonant UnmarkedVocalFolds UnmarkedPlace UnmarkedManner PulmonicEgressive)))
